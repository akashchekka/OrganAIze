...
class TestSpawnerSpawn:
    ...
    @pytest.mark.asyncio
    async def test_spawn_new_agent(self, genesis_blueprint):
        ...
        result = await spawner.spawn(genesis_blueprint, spec)
    ...
    @pytest.mark.asyncio
    async def test_spawn_denied_global_cap(self, genesis_blueprint):
        spawner = self._make_spawner()
        from config import MAX_GLOBAL_AGENTS
        spawner.agents_created = MAX_GLOBAL_AGENTS  # At cap

        result = await spawner.spawn(genesis_blueprint, {"task": "test"})
        assert result is None

    @pytest.mark.asyncio
    async def test_spawn_denied_budget_exhausted(self):
        bp = AgentBlueprint(
            agent_id="parent-1",
        ...
        result = await spawner.spawn(bp, {"task": "test"})
        assert result is None

    @pytest.mark.asyncio
    async def test_child_budget_halves(self, genesis_blueprint):
        spawner = self._make_spawner()
        ...
        child = await spawner.spawn(genesis_blueprint, spec)
    ...
    @pytest.mark.asyncio
    async def test_child_depth_increments(self, genesis_blueprint):
        spawner = self._make_spawner()

        child = await spawner.spawn(genesis_blueprint, {
            "name": "Child", "role": "engineer", "task": "test", "success_criteria": "done",
        })
        assert child.depth == 1  # genesis is depth 0

    @pytest.mark.asyncio
    async def test_spawn_agent_denied_for_deep_child(self):
        ...
        child = await spawner.spawn(parent, {
    ...
    @pytest.mark.asyncio
    async def test_agents_created_counter_increments(self, genesis_blueprint):
        spawner = self._make_spawner()
        assert spawner.agents_created == 0

        await spawner.spawn(genesis_blueprint, {
